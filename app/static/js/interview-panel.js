    ...
    backdrop.innerHTML = `
        ...
        <div class="interview-drawer" role="dialog" aria-modal="true" aria-labelledby="iv-panel-title">
            <div class="interview-drawer-header">
                <h2 id="iv-panel-title">${t('interviews.interviewDrawer.title')}</h2>
                <button class="interview-drawer-close" id="iv-panel-close" aria-label="${t('a11y.close')}">&times;</button>
            </div>
...
async function loadInterviewPanelData(backdrop, roundId, jobId) {
    ...
    try {
        ...
        if (!round) {
            panel.innerHTML = `<div class="empty-state empty-state-compact"><div class="empty-state-title">${t('interviews.interviewDrawer.notFound')}</div></div>`;
            return;
        }

        renderInterviewPanelLeft(panel, round, job);
    } catch (err) {
        panel.innerHTML = `<div class="empty-state empty-state-compact"><div class="empty-state-title">${t('errors.loadFailed')}</div><div class="empty-state-desc">${escapeHtml(apiErrorMessage(err))}</div></div>`;
    }
}
...
function renderInterviewPanelLeft(panel, round, job) {
    ...
    const formatPanelDate = (isoStr) => {
        if (!isoStr) return t('interviews.interviewDrawer.notScheduled');
        return i18n.formatWeekdayDateLong(isoStr);
    };

    const formatPanelTime = (isoStr, durationMin) => {
        if (!isoStr) return '';
        const start = i18n.toDate(isoStr);
        const end = new Date(start.getTime() + (durationMin || 60) * 60000);
        return `${i18n.formatTime(start)} — ${i18n.formatTime(end)}`;
    };
    ...
    panel.innerHTML = `
        ...
        <div class="interview-details-grid">
            ${round.scheduled_at ? `
                <span class="detail-label">${t('interviews.interviewDrawer.date')}</span>
                <span class="detail-value">${formatPanelDate(round.scheduled_at)}</span>
                <span class="detail-label">${t('interviews.interviewDrawer.time')}</span>
            ...
            ` : ''}
            ${round.location ? `
                <span class="detail-label">${t('interviews.interviewDrawer.location')}</span>
        ...
        ${round.notes ? `
            <div style="margin-top:16px">
                <div style="font-size:0.8125rem;font-weight:500;color:var(--text-tertiary);margin-bottom:4px">${t('interviews.interviewDrawer.notes')}</div>
        ...
        ${round.status === 'scheduled' ? `
            <div style="display:flex;gap:8px;margin-top:20px">
                <button class="btn btn-primary btn-sm" id="iv-panel-complete">${t('interviews.interviewDrawer.markComplete')}</button>
                <button class="btn btn-secondary btn-sm" id="iv-panel-cancel-round">${t('interviews.interviewDrawer.cancelRound')}</button>
            </div>
        ` : ''}
    ...
    if (completeBtn) {
        completeBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                showToast(t('interviews.interviewDrawer.markedComplete'), 'success');
    ...
    if (cancelBtn) {
        cancelBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                showToast(t('interviews.interviewDrawer.cancelled'), 'success');
                loadInterviewPanelData(panel.closest('#interview-panel'), round.id, round.job_id);
            } catch (err) {
                showToast(apiErrorMessage(err), 'error');
                cancelBtn.disabled = false;
                cancelBtn.textContent = t('interviews.interviewDrawer.cancelRound');
            }
        });
    }
